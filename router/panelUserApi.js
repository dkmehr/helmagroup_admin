...
const UpdateHesabfaCustomer = require('../middleware/UpdateHesabfaCustomer');
...
router.post('/fetch-user',jsonParser,async (req,res)=>{
    ...
    try{ 
        ...
        const profileIds = userData.profile
        var profileDetail = []
...
router.post('/update-client',jsonParser,auth,async (req,res)=>{
    ...
    try{
        ...
        if(userNew.cCode){
            resultUser = await UpdateHesabfaCustomer(userNew)
        }
        
       res.json({data:userNew,update:userData,success:"تغییرات اعمال شدند",resultUser})
    }
    catch(error){
        res.status(500).json({message: error.message})
    } 
...
router.post('/create-customer',jsonParser,async (req,res)=>{
    ...
    const data=req.body
    data.phone = EnNumber(req.body.phone)
    data.meliCode = EnNumber(req.body.meliCode)
    data.postalCode = EnNumber(req.body.postalCode)
    data.username = data.sName+ " "+data.cName
    data.agent = agent
    try{
        const checkUser = await customer.findOne({phone:data.phone})
        if(checkUser)
            return res.status(400).json({error:"کاربر در سیستم موجود است"})
